...
public class PartInvoice {
    ...
    //***************************************************************************
    //This will use a CisUtility object that can be set to use either console or gui.
    //It will not be encoded in the json strings.
    //***************************************************************************
    private transient CisUtility cisUtility ;
    public PartInvoice(CisUtility cisUtility) {
    ...
    public void getInformation() {
        ...
        double priceInput = cisUtility.getInputDouble("Enter part price: ");
        setPartPrice(priceInput);
        ...
        // Enhanced quantity handling
        int quantityInput = cisUtility.getInputInt("Enter part quantity: ");
        setPartQuantity(quantityInput);

        invoiceID = cisUtility.getInputString("Enter invoice ID: ");
        // Modified tax rate handling
        double taxInput = cisUtility.getInputDouble("Enter tax rate (e.g. 15 for 15%): ");
        if (taxInput > 1) {
            taxInput = taxInput / 100.0; // Convert percentage to decimal
        }
        setTaxRate(taxInput);
        calculateTotal();
    }
